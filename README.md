...
API. Скрипт включает локальный OpenAI-compatible режим Cursor, записывает ключ,
прописывает base URL и регистрирует реальные model ID из каталога Vibemode.
...
2. находит установленное приложение Cursor;
3. проверяет, что Cursor закрыт;
4. включает локальный OpenAI-compatible режим Cursor;
5. делает backup базы;
6. записывает ключ в `cursorAuth/openAIKey`;
7. выставляет `openAIBaseUrl = https://api.vibemod.pro/v1`;
8. включает `useOpenAIKey = true`;
9. получает список моделей Vibemode через `/v1/models`;
10. добавляет в Cursor все модели, которые вернул API;
11. обновляет все найденные режимы выбора модели в Cursor;
12. проверяет API через `/v1/models`.
...
В приложении Cursor включается локальный OpenAI-compatible режим. Это нужно для
нового агентного окна Cursor, иначе Cursor может отправлять named models через
свой backend и показывать ошибку тарифа даже при правильном Vibemode key.
...
Если Cursor установлен в read-only месте, например в системном Linux-пути,
скрипт создаст отдельный пользовательский запуск `cursor-vibemode` и применит
патч туда. В обычных writable-установках Cursor патчится напрямую.
...
`repair` заново применяет ключ, base URL, `useOpenAIKey` и каталог моделей. Это
полезно после обновления Cursor или если модельные настройки сбились.
...
- `deepseek-*`, `glm-*`, `kimi-*`, `mimo-*` проверяются через `/v1/chat/completions`;
- `minimax-m3`, `qwen3.7-*`, `vibe-lite-1.5` проверяются через `/v1/messages`.

Cursor напрямую работает с одним API-режимом на весь base URL. Поэтому
Messages-модели добавляются в список моделей, но для их запуска из Cursor нужен
отдельный адаптер. Chat/Responses-модели работают напрямую.
