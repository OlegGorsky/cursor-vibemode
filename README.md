...
`cursor-vibemode` настраивает Cursor на работу через Vibemode. Скрипт включает
нужный режим Cursor, записывает ключ и подключает модели из каталога Vibemode.
...
Если ключ уже был сохранен, можно просто нажать Enter и использовать его снова.
...
4. включает нужный локальный режим Cursor;
5. делает backup базы;
6. записывает ключ в `cursorAuth/openAIKey`;
7. настраивает подключение Cursor к Vibemode;
...
В приложении Cursor включается локальный режим. Это нужно для нового агентного
окна Cursor, иначе Cursor может отправлять named models через свой backend и
показывать ошибку тарифа даже при правильном Vibemode key.
...
Запустить глубокую проверку моделей:
...
`repair` заново применяет ключ, настройки подключения и каталог моделей. Это
